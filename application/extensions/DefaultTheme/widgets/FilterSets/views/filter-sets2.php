...
<div class="filter-sets-widget<?= $isInSidebar ? ' sidebar-widget' : ''; ?>">
    ...
    <div class="filters" id="<?= $id; ?>">
        ...
        $urlParams = [  //ID категории ( [last_category_id] => 9   Трусы)
            '@category',
        ...
        <form action="<?= Url::toRoute(['@category', 'last_category_id' => $urlParams['last_category_id']]); ?>" method="post" class="filter-form">
            ...
            <?php if ($checkedIds):?>
                <div id="selected-filters">
                    ...
                    <div id="clean"><button type="submit" id="button-reset" class="btn btn-primary btn-sm">Очистить фильтры</button></div>
...
TPL;
                ...
                echo array_reduce($filterSets, // ID свойства ([property_id] => 10
                    function ($result, $item) use ($urlParams, $_tplHtml)
                    {
                        ...
                        $selections = \app\models\PropertyStaticValues::getValuesForFilter(
                            ...
                            true,
                            $productsFilteringMode=5 // Моё изменение
                        );
                        if (empty($selections)) {
                            return $result;
                        }
            ...
            <div class="filter-actions">
                <button type="submit" class="btn btn-success btn-filter-show btn-sm"><?= Yii::t('app', 'Show'); ?></button>
            </div>
            ...
            if (Yii::$app->request->isAjax) {
                echo '<script>
                    ...
                    var valSelect = $("' . $checkedIds . '").siblings().clone().addClass(\'sel-prop-link\').prepend(\' <span class="glyphicon glyphicon-eye-close"></span>  \');
