 ...
 */
class Category extends ActiveRecord implements ImportableInterface
{
    use GetImages;
    ...
    {
        // Yii::setAlias('@category', '/shop/product/list');
...
// $view_cat - лямда функция для создания массива категорий, который будет передан в отображение
        ...
        $view_cat =
            function ($data, $parent_id = 1) use ( & $view_cat)
            {
                $result = NULL;
                ...
                for ($i = 0; $i < count($data[$parent_id]);$i++) {
                    $result[] = ['label' => $data[$parent_id][$i]['name'],
                        ...
                        'url' => 'shop/'.$data[$parent_id][$i]['slug'],

                        'url' => Url::toRoute(
                            [
                                '@category',
                                // 'category_group_id' => $category->category_group_id,
                                'last_category_id' => $data[$parent_id][$i]['id'],
                            ]
                        ),
