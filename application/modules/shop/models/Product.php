...
class Product extends ActiveRecord implements ImportableInterface, ExportableInterface
{
    ...
    public function attributeLabels()
    {
        return [
            ...
            'optionsQuantity' => Yii::t('app', 'Кол-во всех'),
    ...
    public static function findById($id, $isActive = 1)
    {
        ...
        if (!isset(static::$identity_map[$id])) {
            ...
            if (false === $model = Yii::$app->cache->get($cacheKey)) {
                $model = static::find()->where(['id' => $id])->with('images')->orderBy(['date_added' => SORT_DESC]);
                if (null !== $isActive) {
                    if ($permiss != 'admin' && $permiss != 'manager') {
                        $model->andWhere(['active' => $isActive])->orderBy(['date_added' => SORT_DESC]);
                    }
                }
    ...
    /**
     * @return полномочия пользователя (admin, manager и тд)
     */
    public static function getPermission()
    {
        if(isset(Yii::$app->user->id)){
            $permit = AuthAssignment::findOne([
                'user_id' => Yii::$app->user->id,
            ]);
            if (isset($permit->item_name)){
                return $permit->item_name;
            }
        }
    }
    ...
    {
        $permiss= static::getPermission(); // Определяем роль пользователя
    ...
    {
        return $this->hasOne(WarehouseProduct::className(), ['product_id' => 'id'])
    ...
    {
        $parentsID= []; //Массив ID разновидностей продукта
        foreach ($this->options as $parentId) {
            $parentsID[] = $parentId->id;
        }
        $count = WarehouseProduct::find()  //Количество товаров одного наименования
        ->select([
            'warehouse_product.in_warehouse', // select all customer fields
        ])
            ->where(['in', 'product_id', $parentsID])
            ->andWhere(['in', 'warehouse_id', 1]) // 1 - основной склад
            ->sum('[[in_warehouse]]');
        return $count;
    }
    ...
    public function afterActive()
    {
        ...
        Product::updateAll(['active' => 1], $where);
    }

    public function afterInActive()
    {
        $where = ['parent_id' => $this->id];
        Product::updateAll(['active' => 0], $where);
    }

    public function priceChange() 
    {
           $where = ['parent_id' => $this->id];
           Product::updateAll(['price' => $this->price], $where);
    }
    ...
    public function processImportAfterSave(array $fields, $multipleValuesDelimiter, array $additionalFields)
    {
        ...
        if (isset($additionalFields['countValue'])) {
            if (isset($additionalFields['countValue']['processValuesAs'],$fields['countValue'])) {
                $customer = WarehouseProduct::findOne([
                    'product_id' => $this->id,
                ]);
                if ($customer) {
                    if ($customer->in_warehouse != $fields['countValue']){
                        $customer->in_warehouse = $fields['countValue'];
                        $customer->save();
                    }
                }else{
                    // create new record with default values
                    $remain = new WarehouseProduct();
                    $remain->warehouse_id = 1;
                    $remain->product_id = $this->id;
                    $remain->save();
                    $customer = WarehouseProduct::findOne([
                        'product_id' => $this->id,
                    ]);
    ...
    public function getAdditionalFields(array $configuration)
    {
        ...
        if (isset($configuration['subcategory'], $configuration['subcategory']['processValuesAs'])
            && $configuration['subcategory']['enabled']
        ) {
            $result['subcategory'] = ArrayHelper::getColumn($this->subcategory , $configuration['subcategory']['processValuesAs']);
        }
    ...
    public static function filteredProducts(
        ...
        if (null === $object = BaseObject::getForClass(static::className())) {
            throw new ServerErrorHttpException('BaseObject not found.');
        }
        ...
        $query = static::find()->with('images')->orderBy(['date_added' => SORT_DESC]);
        ...
        $fathersArr = $query->all();

        if(!empty($fathersArr[0]['fathers'])){   // Показывать продукты с вариантами и без
            //if(isset($fathersArr[0])){  // Показывать только подукты с детками
            $query->select('pr.*')
                ->from('product')
                ->join('LEFT JOIN', 'product pr', 'pr.id = product.parent_id');
            $q = $query->all(); //Массив родителей выбранных  в фильтре продуктов
            $j = count($q); // Количество родителей
            $i = 0;
            $arrParentId = []; // Массив id родителей
            while ($i < $j) {
                array_push($arrParentId , $q[$i]['id']);
                $i++;
            }
            $query = static::find()
