 ...
 * @package app\widgets
 */
class CartImageWidget extends Widget
{
    /**
    ...
    public $noImageOnEmptyImages = true;
    ...
    public function run()
    {
        ...
        if ($result === false) {
            if ($this->offset > 0 || !is_null($this->limit)) {
                $images = $this->model->getCartImage()->limit($this->limit)->offset($this->offset)->all();
               $val = $this->model->getVal();
            } else {
