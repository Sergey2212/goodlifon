...
class PropertyStaticValues extends ActiveRecord
{
    ...
    public static function getValuesForFilter($property_id, $category_id, $properties, $multiple = false, $productsFilteringMode = "")
    {
        ...
        if (false === $allSelections = Yii::$app->cache->get($cacheKey)) {
            ...
            if (false === empty($properties)) {
                foreach ($properties as $propertyId => $propertyStaticValues) {
                    ...
                    $subQueryOptimisation = Yii::$app->db->cache(function($db) use ($subQuery) {
                        ...
                        'tags' => [
                            ActiveRecordHelper::getCommonTag(ObjectStaticValues::class),
                        ]
                    ]));
			...
			/**
			* @return массив  [in_warehouse] => 0 
			*                 [property_static_value_id] => 96
			* 		  по ID товаров
			*/
			$arrProductInWarehouse = Product::find()
			->select('warehouse_product.in_warehouse, object_static_values.property_static_value_id')
			->leftJoin('product_category', '`product_category`.`object_model_id` = `product`.`id`')
			->leftJoin('category', '`category`.`id` = `product_category`.`category_id`')
			->leftJoin('object_static_values', '`object_static_values`.`object_model_id` = `product`.`id`')
			->leftJoin('warehouse_product', '`warehouse_product`.`product_id` = `product`.`id`')
			->where(['product.id' => self::getArrProdId($selected)])
			->andWhere(['product.active' => 1])
			->andWhere(['warehouse_product.warehouse_id' => 1])
			->andWhere(['product_category.category_id' => $category_id])
			->asArray()
			->all();
            ...
            foreach ($allSelections as $index => $selection) {
                ...
                //$allSelections[$index]['in_warehouse'] = $selection['id'];
				$allSelections[$index]['in_warehouse'] = self::countProduct($arrProductInWarehouse, $selection['id'], $category_id);
               $i++;
            }
            ...
            if (null !== $allSelections) {
                Yii::$app->cache->set(
                    ...
                    new TagDependency(
                        [
                            'tags' => [
                                ActiveRecordHelper::getCommonTag(PropertyStaticValues::class),
    ...
    //Ищет по размеру или цвету ID продуктов у которых такой цвет или размер
    public function getProductsId()
    {
        return $this
            ->hasMany(Product::class, ['id' => 'object_model_id'])
            ->viaTable('object_static_values', ['property_static_value_id' => 'id']);
    }


    /**
     * @param $propertyStaticValuesId
	 * [266, 267, 132, 129, 96]  38(266) 40(267) Чёрный(132) Бежевый(129) Орхидея(96)
	 * @return массив ID продуктов
     */
	 public function getArrProdId($propertyStaticValuesId)
    {
		$arrProductsId = [];
        $query = ObjectStaticValues::find()
            ->select('object_model_id')
            ->where(['property_static_value_id' => $propertyStaticValuesId])
            ->asArray()
            ->all();
        foreach ($query as $index => $value) {
            $arrProductsId[] = $value['object_model_id'];
        }
        return $arrProductsId;
    }

    
	public function countProduct($arrProductInWarehouse, $propertyStaticValuesId, $categoryId)
	{
		$sum = 0;
		foreach ($arrProductInWarehouse as $key => $value) {
			if ($value['property_static_value_id'] == $propertyStaticValuesId) {
				$sum += $value['in_warehouse'];
			}
		}
		return $sum;
	}
